			...
			<tbody class="divide-y divide-slate-100">
				{#each data.webhooks as row (row.delivery.id)}
					<tr>
						...
						<td class="table-cell max-w-[16rem] truncate text-xs text-red-600">{row.delivery.errorMessage ?? '—'}</td>
			...
			<tbody class="divide-y divide-slate-100">
				{#each data.payments as row (row.payment.id)}
					<tr>
						...
						<td class="table-cell text-xs text-red-600">{row.payment.failureMessage ?? row.payment.failureCode ?? '—'}</td>
			...
			<tbody class="divide-y divide-slate-100">
				{#each data.messages as row (row.message.id)}
					<tr>
						...
						<td class="table-cell max-w-[20rem] truncate text-xs text-red-600">{row.message.errorMessage ?? row.message.errorCode ?? '—'}</td>
