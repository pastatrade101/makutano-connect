	...
	<div class="mx-auto max-w-[1240px] px-4 py-[90px] sm:px-6 lg:px-10">
		...
		<div class="mt-24 overflow-hidden rounded-[24px] border border-brand-200 bg-[#0e335d] text-white shadow-[0_35px_90px_rgba(14,51,93,0.20)]">
			<div class="grid items-center gap-10 p-6 sm:p-9 lg:grid-cols-[0.8fr_1.2fr] lg:p-12">
				<div><span class="text-[10px] font-bold tracking-[0.17em] text-blue-200 uppercase">03 · Payment infrastructure</span><h3 class="mt-4 text-3xl leading-tight font-bold tracking-[-0.035em] text-white">Payment requests that don’t end in a dead end.</h3><p class="mt-4 text-sm leading-7 text-blue-100/75">Tell the customer how to pay, capture their report, require verification, update the transaction and notify them—all without pretending a button proves money arrived.</p></div>
				<div class="rounded-2xl border border-white/15 bg-white p-5 text-slate-700 shadow-2xl"><div class="flex items-start justify-between gap-3"><div><p class="text-[9px] font-bold tracking-wider text-slate-400 uppercase">Payment requested</p><p class="mt-1 text-2xl font-bold text-slate-900">TZS 850,000</p><p class="mt-1 text-[10px] text-slate-400">M-Pesa · GFA-BK-2026-00128</p></div><span class="rounded-full bg-warning/10 px-2.5 py-1 text-[9px] font-semibold text-[#a9780c]">Awaiting verification</span></div><div class="mt-5 grid gap-2 sm:grid-cols-5">{#each [{ t: 'Requested', s: 'How to pay', c: 'done' }, { t: '“I have paid”', s: 'Customer report', c: 'done' }, { t: 'Verify', s: 'Staff action', c: 'active' }, { t: 'Booking updated', s: 'Domain service', c: 'later' }, { t: 'Customer notified', s: 'WhatsApp', c: 'later' }] as item, index (item.t)}<div class="rounded-xl border p-3 {item.c === 'done' ? 'border-success/20 bg-success/5' : item.c === 'active' ? 'border-warning/30 bg-warning/5' : 'border-slate-200 bg-slate-50'}"><span class="text-[8px] font-bold {item.c === 'done' ? 'text-success' : item.c === 'active' ? 'text-[#a9780c]' : 'text-slate-300'}">0{index + 1}</span><div class="mt-5 text-[9px] font-semibold text-slate-700">{item.t}</div><div class="mt-0.5 text-[8px] text-slate-400">{item.s}</div></div>{/each}</div></div>
			</div>
		</div>
...
<section class="border-b border-slate-100 bg-[#0a203a] text-white">
	<div class="mx-auto max-w-[1240px] px-4 py-20 sm:px-6 lg:px-10 lg:py-24">
		<div class="max-w-2xl"><p class="text-[10px] font-bold tracking-[0.18em] text-blue-200 uppercase">Customer journey</p><h2 class="mt-3 text-3xl font-bold tracking-[-0.035em] text-white sm:text-4xl">From first message to confirmed customer.</h2></div>
		<div class="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-8">{#each [{ n: '01', t: 'WhatsApp message' }, { n: '02', t: 'Enquiry' }, { n: '03', t: 'Quotation' }, { n: '04', t: 'Payment request' }, { n: '05', t: '“I have paid”' }, { n: '06', t: 'Verification' }, { n: '07', t: 'Booking confirmed' }, { n: '08', t: 'Customer notified' }] as item (item.n)}<div class="relative rounded-xl border border-white/10 bg-white/5 p-3.5 backdrop-blur"><span class="text-[8px] font-bold text-blue-300">{item.n}</span><p class="mt-6 text-[10px] leading-4 font-semibold text-white/85">{item.t}</p></div>{/each}</div>
	</div>
</section>
