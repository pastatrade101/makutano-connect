...
const usable = (c: Candidate): boolean =>
	...
	c.width >= 900 &&
	!/\b(map|flag|logo|coat of arms|diagram|chart|seal|locator|sign|signboard|board|notice|gate|entrance|poster|relief|topograph\w*|satellite|3d|model|render|sketch|drawing|stamp|banknote|coin|graph|plot|profile)\b/i.test(
		c.title
	) &&
...
for (const d of rows.slice(0, Number.isFinite(limit) ? limit : undefined)) {
	try {
		...
		if (!picks.length) {
			console.log(`SKIP  ${d.slug} — nothing usably licensed`);
			skipped++;
			continue;
		}
		...
		if (!img.ok) {
			console.log(`SKIP  ${d.slug} — download ${img.status}`);
			skipped++;
			continue;
		}
